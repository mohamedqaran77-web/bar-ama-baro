# Bar ama Baro – Master Task List

## Milestone 0: Foundation
- [x] Create monorepo folders
- [x] Add AGENTS.md
- [x] Add README files
- [x] Add environment examples
- [x] Add coding standards
- [x] Add database schema

## Milestone 1: Auth & Roles
- [x] Register screen
- [x] Login screen
- [x] OTP verification placeholder
- [x] Forgot password
- [ ] Student role
- [ ] Teacher role
- [ ] Parent role
- [ ] Admin role

## Milestone 2: Public Learning Marketplace
- [ ] Home page
- [ ] Courses page
- [ ] Course details page
- [ ] Teachers page
- [ ] Teacher profile page
- [ ] Categories page

## Milestone 3: Student MVP
- [ ] Student dashboard
- [ ] My courses
- [ ] Lesson player
- [ ] Progress tracking
- [ ] Quiz attempt
- [ ] Certificates

## Milestone 4: Teacher MVP
- [ ] Teacher dashboard
- [ ] Create course
- [ ] Upload lesson metadata
- [ ] View students
- [ ] Earnings placeholder

## Milestone 5: Admin MVP
- [ ] Admin login guard
- [ ] User list
- [ ] Teacher approval
- [ ] Course approval
- [ ] Payment verification
- [ ] Reports

## Milestone 6: Business Features
- [ ] Manual mobile money payment reference
- [ ] Enrollment after admin approval
- [ ] Teacher commission calculation
- [ ] Certificate QR verification

## Milestone 7: AI Beta
- [ ] AI tutor limited to course content
- [ ] AI quiz generator
- [ ] AI lesson summary
